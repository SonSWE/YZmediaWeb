...
export const useCorporateActionApi = (Corp_Type, controller) => {
  ...
  return {
    SearchData: (keysearch, from, to, orderBy) => {
      return apiConnection.httpRequest(
        "POST",
        `api/sh/corporate-action/search`,
        null,
        {
          p_corp_type: Corp_Type,
    ...
    SearchDataEsop: (keysearch, from, to, orderBy) => {
      return apiConnection.httpRequest(
        "POST",
        `api/sh/corporate-action/search-esop`,
        null,
        {
          p_com_id: getUserFromStorage()?.Reference_Id,
          p_user_name: get_User_Name(),
    ...
    GetByIdAndComId: (id, com_id) => {
      return apiConnection.httpRequest(
        "GET",
        `api/sh/corporate-action/get-by-id`,
        null,
        { p_com_id: com_id, p_corp_action_id: id },
        true
      );
    },
    SearchDataByShareHolder: (keysearch, from, to, orderBy) => {
      return apiConnection.httpRequest(
        "POST",
        `api/sh/corporate-action/search-by-share-holder`,
        null,
        {
          p_share_holder_id: getUserFromStorage()?.Reference_Id,
          p_user_name: get_User_Name(),
          keysearch: keysearch,
          from: from,
          to: to,
    ...
    insert: (prop) => {
      ...
      return apiConnection.httpRequest(
        "POST",
        "api/sh/corporate-action/insert",
        prop,
        null,
        true
      );
